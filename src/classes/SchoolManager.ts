...
export default class SchoolManager {
	...
	public setLecturerToTeam(
		...
		const foundLecturer: Lecturer | undefined = this.lecturers.find(
			(lecturer: Lecturer) => {
				if (
	...
	public setStudentToTeam(
		...
		const foundStudent: Student | undefined = this.students.find(
			(student: Student) => {
	...
	public getStudentAgeFromId(id: string): number {
		...
		if (foundStudent && foundStudent !== undefined) {
			return moment().diff(
				moment(foundStudent.dateOfBirth, "DD/MM/YYYY"),
				"years"
			);
		}
		return 0;
